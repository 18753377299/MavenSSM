...
//		HttpResponseAddressReset result = mapService.getLongAndLat_test(riskMapAddress);
	...
	*//**
	 ...
	 *//**
	  * @author 周东旭
	  * @功能 获取所有的riskCode*//*
	 @RequestMapping(value="/queryAllRiskCode",method = {RequestMethod.POST,RequestMethod.GET})
	 @ResponseBody
	 public AjaxResult queryAllRiskCode(@RequestParam("comCode") String comCode) {
		AjaxResult ajaxResult = new AjaxResult();
		List<Code> riskCodeMap = null;
		try {
			riskCodeMap=mapService.queryRiskCode("",comCode);
			ajaxResult.setData(riskCodeMap);
		} catch (Exception e) {
	...
	*//**
	...
	@ResponseBody AjaxResult createTyphoonWordCurrent(HttpServletRequest req,@RequestBody TfWordReqVo  vo) {
		...
		String userCode=userInfo.getUserCode();
		AjaxResult ajaxResult = mapService.createTyphoonWordCurrent(vo.getTfbh(),vo.getImgData(),userCode);
		return ajaxResult;
	}
	...
	*//**
	...
	@ResponseBody AjaxResult createTyphoonWordYJ(HttpServletRequest req,@RequestBody TfWordReqVo  vo) {
		UserInfo userInfo = (UserInfo) req.getAttribute("userInfo");
		String userCode=userInfo.getUserCode();
		AjaxResult ajaxResult = mapService.createTyphoonWordYJ(vo.getTfbh(),vo.getImgData(),userCode);
		return ajaxResult;
	}
	...
	*//**
	...
	public AjaxResult queryTargetForTfbh(@RequestParam("tfbh") String tfbh, 
		...
		AjaxResult ajaxResult = new AjaxResult();
		ajaxResult = mapService.queryTargetForTfbh(tfbh, target);
		return ajaxResult;
	}
