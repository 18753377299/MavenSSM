-- 资产表
-- 资产维护表
drop table if exists t_ass_defend;
CREATE TABLE t_ass_defend(
...
asset_current_status_date		datetime  COMMENT '当前状态日期',
...
-- 资产人员关联表
drop table if exists t_ass_person;
CREATE TABLE t_ass_person(
    asset_no	 varchar(22)  	NOT NULL	COMMENT'资产编号',
...
CREATE TABLE  t_ass_appr_rule_config(
    ...
    asset_check_object	 varchar(1)		COMMENT'审核对象',
    asset_nature	 varchar(1)		COMMENT'资产性质',
    ...
    asset_defend_date	 datetime  		COMMENT'维护日期',
    asset_defend_code	 varchar(10)  		COMMENT'维护人员',

    is_validate_flag	 varchar(1)  		COMMENT'审核是否有效标志',
    insert_time_for_his	 datetime	NULL  DEFAULT NOW()	COMMENT'插入时间',
    operate_time_for_his	datetime	NULL  DEFAULT NOW()	COMMENT'更新时间',
    PRIMARY KEY (asset_check_type,asset_check_person_code,asset_check_object)
